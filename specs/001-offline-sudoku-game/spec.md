...
- Q: Should the number pad interface (for mouse input) also display keyboard hints for numbers 1-9? → A: No, number pad is for mouse users only; keyboard users type 1-9 directly without needing the visual pad
...
- Q: Where should the number pad be positioned? → A: Number pad positioned to the right of grid
...
1. **Given** the player is viewing the game, **When** they see the Notes Mode toggle above the number pad, **Then** it displays as a segmented control with two options: "FILL" (blue, active by default) and "NOTES" (purple when active)
...
- **FR-008**: System MUST support mouse input for all game interactions including cell selection; system MUST display an on-screen number pad positioned to the right of the grid for mouse-based number entry (number pad buttons do not display keyboard hints as they are for mouse users only)
...
- **FR-011**: System MUST provide a toggleable "Notes Mode" via segmented control (FILL | NOTES) positioned above number pad, with 'N' key hotkey; when NOTES is active, the toggle shows purple highlight, selected cell borders become purple, and number entries (1-9) toggle candidate marks (add/remove) in empty cells instead of entering final values; when FILL is active (default), the toggle shows blue highlight, selected cell borders are blue, and number entries fill cells with final values
